...
class XROR:
    ...
    def addEventType(
        self, id=None, name=None, attr=None, floatData=None, otherData=None
    ):
        """
    ...
    def pack(self):
        ...
        if "timestamp" in data["info"]:
            data["info"]["timestamp"] = datetime.datetime.fromtimestamp(
                data["info"]["timestamp"], datetime.timezone.utc
            )
        ...
        for i in range(len(data["events"])):
            if "floatData" in data["events"][i]:
                data["events"][i]["floatData"] = compress(
                    data["events"][i]["floatData"]
                )
    ...
    def toBSOR(self):
        ...
        addString(
            self.data["info"]["user"]["name"]
            if "name" in self.data["info"]["user"]
            else ""
        )
        addString(self.data["info"]["software"]["runtime"])
        addString(self.data["info"]["software"]["api"])
        addString(
            self.data["info"]["hardware"]["devices"][0]["name"]
            if "name" in self.data["info"]["hardware"]["devices"][0]
            else ""
        )
        addString(
            self.data["info"]["hardware"]["devices"][2]["name"]
            if "name" in self.data["info"]["hardware"]["devices"][2]
            else ""
        )
        ...
        for note in notes:
            ...
            if note["eventType"] == 0 or (
                note["eventType"] == 1
                and self.data["info"]["software"]["runtime"] != "oculus"
            ):
    ...
    def fromTilt(XROR, data):
        ...
        xror.addDevice(
            name="BRUSH", type="OTHER", axes=["x", "y", "z", "i", "j", "k", "1", "p"]
        )
        xror.addEventType(
            id="stroke",
            name="Stroke",
    ...
    def fromBSOR(XROR, data, addFPS=False, addOrder=False, addGarbage=False):
        ...
        xror.addDevice(
            name=bsor.info.controller.replace("right", "left")
            .replace("Right", "Left")
            .replace("RIGHT", "LEFT"),
            type="CONTROLLER",
            joint="HAND_LEFT",
        ...
        for attr in [
            ...
            if getattr(bsor.info, attr):
                xror.data["info"]["software"]["activity"][attr] = getattr(
                    bsor.info, attr
                )
        ...
        if addOrder:
            ...
            if bsor.info.platform == "oculus":
                xror.addEventType(
                    id="bc", name="Bad Cut", attr=["spawnTime", "noteID", "order"]
                )
                if addGarbage:
            ...
            xror.addEventType(
                id="m", name="Miss", attr=["spawnTime", "noteID", "order"]
            )
            xror.addEventType(
                id="b", name="Bomb Cut", attr=["spawnTime", "noteID", "order"]
            )
        else:
            xror.addEventType(
                id="gc",
        ...
        xror.addEventType(
            id="wh", name="Wall Hit", attr=["energy", "spawnTime", "wallID"]
        )
